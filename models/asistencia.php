...
class Asistencia
{
    ...
    public static function getAllAsistencia($id)
    {
        ...
        while ($asistencia = $req->fetch()) {
            ...
            // echo '<pre>'; // Esto es solo para mejorar la legibilidad del resultado
            // var_dump($asistencias); // O puedes usar print_r($actividades);
            // echo '</pre>';
    ...
    public static function create($persona_id, $actividad_id)
    {
        ...
        try {
            date_default_timezone_set('America/La_Paz'); // Establecer el huso horario de Bolivia
            $horallegada = date("H:i:s");
            
            $query = 'INSERT INTO asistencias (actividad_id, persona_id, horallegada) VALUES (:actividad_id, :persona_id, :horallegada)';
            $req_asistencia = $db->prepare($query);
            $req_asistencia->bindParam(':actividad_id', $actividad_id);
            $req_asistencia->bindParam(':persona_id', $persona_id);
            $req_asistencia->bindParam(':horallegada', $horallegada);
            $req_asistencia->execute();
